...
public class BlogController implements Initializable {
    ...
    @FXML
    private TextField newPostMediaPath;
    @FXML
    ...
    @FXML
    private VBox archiveView;
    @FXML
    private VBox archiveFeedContainer;
    @FXML
    private VBox archiveEmptyBox;
    @FXML
    ...
    @FXML
    private Button notifTabAll;
    @FXML
    private Button notifTabUnread;
    ...
    private boolean canSeePost(Post post) {
        if (hiddenPostIds.contains(post.getIdPost()))
            return false;
        if (aiHiddenPostIds.contains(post.getIdPost()))
            return false;
        String statut = post.getStatut() != null ? post.getStatut().toLowerCase() : "public";
        if (statut.equals("hidden"))
            return false;
        if (statut.contains("private"))
    ...
    private void rebuildArchive() {
        ...
        for (Post p : postsList)
            if (savedPostIds.contains(p.getIdPost()))
                saved.add(p);
        for (Post post : saved)
    ...
    private VBox buildPostCard(Post post, boolean isArchive) {
        ...
        if (post.getMedia() != null && !post.getMedia().isBlank()) {
            try {
                ...
                if (mediaNode != null)
                    inner.getChildren().add(mediaNode);
            } catch (Exception ex) {
        ...
        stats.getChildren().addAll(statLabel("♥ " + reactions.size()), statLabel("·"),
    ...
    private HBox buildCardHeader(Post post, boolean isArchive) {
        ...
        nameL.setStyle(
                "-fx-font-size:13px; -fx-font-weight:bold; -fx-text-fill:#111827; -fx-font-family:'Segoe UI',Arial;");
    ...
    private Node buildVideoPlayer(String path) {
        try {
            ...
            playPause.setStyle(
                    "-fx-background-color:#1E40AF; -fx-background-radius:8; -fx-text-fill:white; -fx-font-size:13px; -fx-min-width:36; -fx-min-height:32; -fx-cursor:HAND;");
    ...
    private HBox buildActionsRow(Post post, List<Reaction> reactions,
        ...
        translateBtn.setOnMouseEntered(e -> {
            if (!translateBtn.isDisabled())
                translateBtn.setStyle(translateBtn.getStyle().replace("-fx-background-color:transparent;",
                        "-fx-background-color:#EFF6FF;"));
        });
        translateBtn.setOnMouseExited(
                e -> translateBtn.setStyle(getTranslateBtnStyle(translatedPostIds.contains(post.getIdPost()))));
    ...
    private VBox buildCommentsSection(Post post, List<Commentaire> comments) {
        ...
        spamLabel.setStyle(
                "-fx-text-fill:#EF4444; -fx-font-size:11px; -fx-font-family:'Segoe UI',Arial; -fx-padding:0 0 6 0;");
    ...
    private VBox buildCommentItem(Commentaire comment, Post post, Label spamLabel) {
        ...
        bubble.setStyle(
                "-fx-background-color:#FFFFFF; -fx-background-radius:4 14 14 14; -fx-padding:8 12 8 12; -fx-effect:dropshadow(gaussian,rgba(0,0,0,0.04),5,0,0,1);");

        Label nameL = new Label(isMyComment ? currentUsername + " (moi)" : "Utilisateur");
        nameL.setStyle(
                "-fx-font-size:12px; -fx-font-weight:bold; -fx-text-fill:#111827; -fx-font-family:'Segoe UI',Arial;");
        ...
        boolean alreadyReactedC = reactionService.getUserReaction(null, comment.getIdCommentaire(),
                currentUserId) != null;
        Button cReactBtn = linkButton((alreadyReactedC ? "♥ " : "♡ ") + cReacts.size(),
                alreadyReactedC ? "#EF4444" : "#6B7280");
        ...
        if (isMyComment) {
            ...
            editItem.setOnAction(e -> {
                ...
                confirmBtn.setOnAction(ev -> {
                    ...
                    if (fieldIdx >= 0)
                        bubble.getChildren().set(fieldIdx, textL);
    ...
    private VBox buildReplyBubble(Commentaire reply, Post post, VBox replyThread,
        ...
        avatar.setAlignment(Pos.CENTER);
        avatar.setStyle(
                "-fx-background-color:#DBEAFE; -fx-background-radius:13; -fx-text-fill:#1E40AF; -fx-font-size:10px; -fx-font-weight:bold;");
        ...
        boolean alreadyReactedR = reactionService.getUserReaction(null, reply.getIdCommentaire(),
                currentUserId) != null;
        Button rReactBtn = linkButton((alreadyReactedR ? "♥ " : "♡ ") + rReacts.size(),
                alreadyReactedR ? "#EF4444" : "#6B7280");
        ...
        if (isMyReply) {
            ...
            editItem.setOnAction(e -> {
                ...
                confirmBtn.setOnAction(ev -> {
                    ...
                    if (fi >= 0)
                        bubble.getChildren().set(fi, textL);
                    bubble.getChildren().remove(editBtns);
                });
                cancelBtn.setOnAction(ev -> {
                    int fi = bubble.getChildren().indexOf(editField);
    ...
    private HBox buildAddCommentForm(Post post, VBox targetList,
        ...
        sendBtn.setStyle(
                "-fx-background-color:#1E40AF; -fx-background-radius:16; -fx-text-fill:white; -fx-font-size:13px; -fx-min-width:32; -fx-min-height:32; -fx-cursor:HAND;");

        Runnable submit = () -> {
            String txt = field.getText().trim();
            if (txt.isEmpty())
                return;
            if (!canPostComment(spamLabel))
                return;
            ...
            if (parentComment == null) {
                ...
                Commentaire reply = new Commentaire(txt, post.getIdPost(), currentUserId,
                        parentComment.getIdCommentaire());
    ...
    private void handleNotifTabAll() {
        ...
        notifTabAll.setStyle(
                "-fx-background-color:transparent; -fx-text-fill:#1E40AF; -fx-font-size:12px; -fx-font-weight:bold; -fx-cursor:HAND; -fx-border-color:#1E40AF; -fx-border-width:0 0 2 0; -fx-border-radius:0; -fx-background-radius:0; -fx-padding:10 12 8 12;");
        notifTabUnread.setStyle(
                "-fx-background-color:transparent; -fx-text-fill:#6B7280; -fx-font-size:12px; -fx-cursor:HAND; -fx-border-color:transparent; -fx-border-width:0 0 2 0; -fx-border-radius:0; -fx-background-radius:0; -fx-padding:10 12 8 12;");
        rebuildNotifList();
    }
    ...
    private void pushNotif(NotificationService.NotifType type,
        ...
        // Otherwise, the recipient will see it when they log in / refresh
        // notifications.
    }
    ...
    private void rebuildNotifList() {
        ...
        if (list.isEmpty()) {
            ...
            empty.setStyle(
                    "-fx-font-size:12px; -fx-text-fill:#9CA3AF; -fx-font-family:'Segoe UI',Arial; -fx-padding:28 0 28 0;");
        ...
        for (NotificationService.NotifRecord notif : list) {
            ...
            row.setOnMouseEntered(
                    e -> row.setStyle(row.getStyle().replace("#FFFFFF", "#F9FAFB").replace("#EFF6FF", "#DBEAFE")));
            row.setOnMouseExited(e -> row.setStyle(notif.read ? "-fx-background-color:#FFFFFF; -fx-cursor:HAND;"
                    : "-fx-background-color:#EFF6FF; -fx-cursor:HAND;"));
            ...
            if (notif.contentPreview != null && !notif.contentPreview.isBlank()) {
                ...
                commentLabel.setStyle(
                        "-fx-font-size:11px; -fx-text-fill:#374151; -fx-font-style:italic; -fx-font-family:'Segoe UI',Arial;");
    ...
    private void handleReact(Post post, Button btn) {
        ...
        if (added) {
            btn.setText("♥  " + count);
            btn.setStyle(
                    "-fx-background-color:#EF4444; -fx-background-radius:8; -fx-text-fill:white; -fx-font-size:12px; -fx-padding:6 12 6 12; -fx-cursor:HAND;");
            ...
            btn.setText("♥  " + count);
            btn.setStyle(
                    "-fx-background-color:transparent; -fx-background-radius:8; -fx-text-fill:#EF4444; -fx-font-size:12px; -fx-padding:6 12 6 12; -fx-cursor:HAND;");
        }
    }
    ...
    private void handleCommentReact(Commentaire comment, Button btn) {
        ...
        if (added) {
            btn.setText("♥ " + count);
            btn.setStyle(
                    "-fx-background-color:transparent; -fx-text-fill:#EF4444; -fx-font-size:11px; -fx-font-weight:bold; -fx-font-family:'Segoe UI',Arial; -fx-padding:2 6 2 0; -fx-cursor:HAND; -fx-border-color:transparent;");
            ...
            btn.setText("♡ " + count);
            btn.setStyle(
                    "-fx-background-color:transparent; -fx-text-fill:#6B7280; -fx-font-size:11px; -fx-font-weight:bold; -fx-font-family:'Segoe UI',Arial; -fx-padding:2 6 2 0; -fx-cursor:HAND; -fx-border-color:transparent;");
        }
    }
    ...
    private void handleEditPost(Post post, VBox cardInner, Label contentLabel) {
        ...
        area.setStyle(
                "-fx-font-size:13px; -fx-background-color:#F9FAFB; -fx-background-radius:8; -fx-border-color:#3B82F6; -fx-border-radius:8; -fx-border-width:1.5; -fx-padding:9;");

        Button saveBtn = new Button("✓  Enregistrer");
        saveBtn.setStyle(
                "-fx-background-color:#22C55E; -fx-background-radius:8; -fx-text-fill:white; -fx-font-size:12px; -fx-padding:6 14 6 14; -fx-cursor:HAND;");

        Button cancelBtn = new Button("Annuler");
        cancelBtn.setStyle(
                "-fx-background-color:transparent; -fx-text-fill:#9CA3AF; -fx-font-size:12px; -fx-padding:6 12 6 12; -fx-cursor:HAND;");
    ...
    private void showStyledModerationDialog(ModerationResult result) {
        ...
        titleL.setStyle(
                "-fx-font-size:15px; -fx-font-weight:bold; -fx-text-fill:#92400E; -fx-font-family:'Segoe UI',Arial;");
        ...
        pctLabel.setStyle("-fx-font-size:12px; -fx-font-weight:bold; -fx-text-fill:" + barColor
        ...
        reasonTitle.setStyle(
                "-fx-font-size:10px; -fx-font-weight:bold; -fx-text-fill:#92400E; -fx-font-family:'Segoe UI',Arial;");
        ...
        okBtn.setStyle(
                "-fx-background-color:#1E40AF; -fx-background-radius:8; -fx-text-fill:white; -fx-font-size:12px; -fx-font-family:'Segoe UI',Arial; -fx-padding:7 22 7 22; -fx-cursor:HAND;");
    ...
    private boolean showStyledDeleteDialog() {
        ...
        titleL.setStyle(
                "-fx-font-size:15px; -fx-font-weight:bold; -fx-text-fill:#991B1B; -fx-font-family:'Segoe UI',Arial;");
        ...
        Label msg = new Label(
                "Êtes-vous sûr de vouloir supprimer cette publication ? Tous les commentaires et réactions associés seront également supprimés.");
        ...
        Button okBtn = (Button) dialog.getDialogPane().lookupButton(ButtonType.OK);
        okBtn.setStyle(
                "-fx-background-color:#EF4444; -fx-background-radius:8; -fx-text-fill:white; -fx-font-size:12px; -fx-font-family:'Segoe UI',Arial; -fx-padding:7 22 7 22; -fx-cursor:HAND;");
        okBtn.setText("Supprimer");
        Button cancelBtn = (Button) dialog.getDialogPane().lookupButton(ButtonType.CANCEL);
        cancelBtn.setStyle(
                "-fx-background-color:transparent; -fx-text-fill:#6B7280; -fx-font-size:12px; -fx-font-family:'Segoe UI',Arial; -fx-padding:7 16 7 16; -fx-cursor:HAND;");
    ...
    private Button ghostButton(String text, String color) {
        ...
        String base = "-fx-background-color:transparent; -fx-background-radius:8; -fx-text-fill:" + color
                + "; -fx-font-size:12px; -fx-font-family:'Segoe UI',Arial; -fx-padding:6 12 6 12; -fx-cursor:HAND;";
        b.setStyle(base);
        b.setOnMouseEntered(
                e -> b.setStyle(base.replace("-fx-background-color:transparent;", "-fx-background-color:#EFF6FF;")));
    ...
    private Button filledButton(String text, String color) {
        ...
        b.setStyle("-fx-background-color:" + color
                + "; -fx-background-radius:8; -fx-text-fill:white; -fx-font-size:12px; -fx-font-family:'Segoe UI',Arial; -fx-padding:6 12 6 12; -fx-cursor:HAND;");
        return b;
    }

    private Button outlineButton(String text, String color) {
        Button b = new Button(text);
        String base = "-fx-background-color:transparent; -fx-background-radius:8; -fx-border-color:#E5E7EB; -fx-border-width:1; -fx-border-radius:8; -fx-text-fill:"
                + color
                + "; -fx-font-size:12px; -fx-font-family:'Segoe UI',Arial; -fx-padding:6 12 6 12; -fx-cursor:HAND;";
        b.setStyle(base);
        b.setOnMouseEntered(
                e -> b.setStyle(base.replace("-fx-background-color:transparent;", "-fx-background-color:#F9FAFB;")));
        b.setOnMouseExited(e -> b.setStyle(base));
        return b;
    }
    ...
    private Button linkButton(String text, String color) {
        ...
        b.setStyle("-fx-background-color:transparent; -fx-text-fill:" + color
                + "; -fx-font-size:11px; -fx-font-weight:bold; -fx-font-family:'Segoe UI',Arial; -fx-padding:2 6 2 0; -fx-cursor:HAND; -fx-border-color:transparent;");
        return b;
    }
